...
class CustomSocialAccountAdapter(DefaultSocialAccountAdapter):
    def pre_social_login(self, request, sociallogin):
        ...
        if not user or not user_email(user):
            logger.warning(
                f"Social login has no user or user has no email attached. sociallogin.user: {user}"
            )
        ...
        try:
            ...
            logger.info(
                f"Found existing user with email {email}, connecting social login."
            )
            # Lenient: connect regardless of verified email addresses
            sociallogin.connect(request, existing_user)
        except get_user_model_ref().DoesNotExist:
            logger.info(
                f"No existing user with email {email}. Proceeding with new user signup."
            )
